...
contract Vault is IERC721Receiver {
    ...
    function _pullAsset(address asset, uint256 id, address origin) internal {
        uint256 originalBalance = balanceOf(asset, id, address(this));

        IERC721(asset).transferFrom(origin, address(this), id);
        _checkTransfer(asset, id, originalBalance, address(this));

        emit VaultPull(asset, id, origin);
    }

    function _pushAsset(address asset, uint256 id, address beneficiary) internal {
        uint256 originalBalance = balanceOf(asset, id, beneficiary);

        IERC721(asset).safeTransferFrom(address(this), beneficiary, id, "");
        _checkTransfer(asset, id, originalBalance, beneficiary);

        emit VaultPush(asset, id, beneficiary);
    }

    function _checkTransfer(address asset, uint256 id, uint256 originalBalance, address checkedAddress) internal view {
        if (originalBalance + 1 != balanceOf(asset, id, checkedAddress)) {
